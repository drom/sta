...
function grid (width, height, dt) {
    var res = ['g'];
    var i;
    res.push(['rect', { x: 0, y: 0, width: width, height: height, stroke: '#13a', fill: 'none' }]);
    for (i = 0; i < width; i += dt) {
        res.push(['line', { x1: i, y1: 0, x2: i, y2: height, stroke: '#13a', 'stroke-dasharray': '5, 5' }]);
    }
    return res;
}
...
function render (data, options) {
    ...
    var width = int(data[data.length - 1].total * dt);
    var height = int(data.length * dv);
